...
public sealed class SmartModelGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateResultMapper(INamedTypeSymbol classSymbol)
    {
        ...
        foreach (IPropertySymbol property in properties)
        {
            ...
            // For nullable properties (T? where T is reference or value type) the null-column
            // branch assigns null, which is valid by definition. For non-nullable reference and
            // value types we leave the property at its model-level default (collection inits,
            // string.Empty, default value-type, etc.) when the column is null — assigning
            // default(T) for non-nullable references would just be null which would warn.
