...
public sealed class SqlServerValueWriter : IValueWriter
{
    public void WriteLiteral(StringBuilder buffer, object? value)
    {
        ...
        switch (value)
        {
            ...
            case DateTime dateTimeValue:
                buffer.Append('\'').Append(dateTimeValue.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture)).Append('\'');
                break;
            case DateTimeOffset dateTimeOffsetValue:
                buffer.Append('\'').Append(dateTimeOffsetValue.ToString("yyyy-MM-dd HH:mm:ss.fff zzz", CultureInfo.InvariantCulture)).Append('\'');
                break;
            case Guid guidValue:
    ...
    public void WriteArrayLiteral(StringBuilder buffer, Array elements, string? elementDatabaseType)
        => throw new NotSupportedException("SQL Server has no array column type.");
}
